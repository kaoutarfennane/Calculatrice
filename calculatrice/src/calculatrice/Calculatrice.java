...
public class Calculatrice {
	...
	private void calcul() {
		...
		/*
		 * switch (operateur) { case "+": resultat += Double.parseDouble(tab); case "-":
		 * resultat -= Double.parseDouble(tab); case "*": resultat *=
		 * Double.parseDouble(tab); case "/" : resultat = resultat /
		 * Double.parseDouble(tab); case "'-'" : resultat*=-1;
		 * 
		 * }
		 */
		if(num1 ) {
		...
		if (operateur.equals("+")) {
			...
			resultat += Double.parseDouble(tab);// Double.parseDouble(String) permet de transformer une chaîne de
												// caractère en double
		}
		if (operateur.equals("-")) {
		...
		if (operateur.equals("/")) {
			...
			// Nécessaire de faire un test si division par 0 pour renvoyer une erreur
